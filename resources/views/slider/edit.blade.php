...
@section('content')
    ...
    <form action="{{route('slider.update',$slider->id)}}" method="POST" class="my-10" enctype="multipart/form-data">
        @csrf
        <input type="text" class="w-full p-2 rounded-lg mt-2" name="priority" placeholder="Enter Priority" value="{{$slider->priority}}">
        @error('priority')
            <span class="text-red-500 -mt-4">* {{$message}}</span>
        @enderror
        <p>Current Image</p>
        <img  class="w-34" src="{{asset('images/slider/'.$slider->photopath)}}" alt="">
